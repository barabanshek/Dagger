...
int ConnectionManager::open_connection(ConnectionId& c_id,
    ...
    if (open_connections_.size() == max_connections_) {
        FRPC_ERROR("Failed to open connection, max number of open connections is reached\n");
        return 1;
    }
    ...
    open_connections_.insert(std::make_pair(
                                 c_id, std::make_pair(dest_addr, flow_id)));

    return 0;
}

int ConnectionManager::add_connection(ConnectionId c_id,
                                      const IPv4& dest_addr,
                                      ConnectionFlowId flow_id) {
    if (open_connections_.size() == max_connections_) {
        FRPC_ERROR("Failed to add connection, max number of open connections is reached\n");
        return 1;
    }
