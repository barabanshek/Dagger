...
TEST_F(ClientServerTestMultithreaded, MultipleDifferentCallsSingleThreadTest) {
    ...
    while(!done && t_out_cnt < ClientServerPair::timeout) {
        ...
        for(int i=0; i<num_of_threads; ++i) {
            done = done & (cqueues[i]->get_number_of_completed_requests() == num_of_it);
        }
    }
    for(int i=0; i<num_of_threads; ++i) {
...
static void test_thread(frpc::RpcClient* client,
                        size_t num_of_it,
                        size_t thread_id,
...
TEST_F(ClientServerTestMultithreaded, MultipleDifferentCallsMultipleThreadTest) {
    ...
    for(int i=0; i<num_of_threads; ++i) {
        std::thread thr = std::thread(&test_thread,
                                      clients[i],
                                      num_of_it,
                                      i+1,
                                      std::ref(expected[i]));
    ...
    for(int i=0; i<num_of_threads; ++i) {
        for(int j=0; j<num_of_it; ++j) {
            ...
            auto to_find = std::make_pair(returned->ret_val,
                                                returned->ret_val_1);
